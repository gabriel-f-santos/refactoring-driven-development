---
name: rdd-improve
description: Use this skill after rdd-port to refactor a parity-correct module for idiomatic style and quality, with the existing characterization tests as a parity safety net. Triggers on "improve X", "refactor X for style", "clean up X after porting", "make X idiomatic". Produces idiomatic code + IMPROVE.progress.md, tests stay green throughout.
---

# rdd-improve — Idiomatic refactor with parity safety net

You are improving the **internal quality** of a module that was ported with parity (`/rdd-port`) but isn't idiomatic. The characterization tests from `/rdd-tests` are your safety net: any refactor that changes observable behavior fails a test, and you revert or fix.
...
The user invokes the skill with a module name (e.g., "/rdd-improve products"). Resolve:
...
- `{artifacts_dir}/{module}/PORT.progress.md` — must show `Status: completed`. If not, instruct the user to finish the port first.
...
1. **Port phase complete.** `PORT.progress.md` shows `Status: completed` and `final_verification` passed. If not, **stop** and instruct the user to finish `/rdd-port {module}` first.
...
- **Test code that violates `/rdd-tests` boundary rules** — tests asserting implementation rather than behavior. Flag for deletion or rewrite.
...
2. **Fix the refactor** (if you can do so within 3 attempts). Apply the fix-loop discipline from `/rdd-port` step 4.4: read error, diagnose, focused fix, re-run. Max 3 attempts. After 3 failures, revert.
...
**Never weaken tests to make them pass.** Never `.skip`, `.only`, `xit`. Never catch and swallow errors. The whole point of `/rdd-improve` is the safety net — undermining it defeats the skill.
...
- Re-run `/rdd-improve {module}` later if more refactors are identified
- Cutover (if not already done) — same flag flip as `/rdd-port` Phase 5

---

## Anti-patterns to avoid

- **Don't refactor without the safety net.** If `/rdd-port` isn't complete, do not run this skill.
